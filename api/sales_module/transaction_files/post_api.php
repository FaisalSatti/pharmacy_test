...
if(isset($_POST['action']) && $_POST['action'] == 'insert'){
  ...
  if($company_code !='0'){
    ...
    $query = "SELECT doc_date,doc_year,co_code,doc_no,doc_type,remarks FROM order_mst WHERE post = 'N' $merge
            union ALL 
            SELECT doc_date,doc_year,co_code,doc_no,doc_type,remarks FROM dc_mst WHERE post = 'N' $merge
            union ALL 
            SELECT doc_date,doc_year,co_code,doc_no,doc_type,remarks FROM rt_mst WHERE post = 'N' $merge";
    // PRINT_R($query); die();
...
//   die();
  ...
  foreach($extract as $detail){
      $value=explode("-",$detail);
        if($value[2]=='RT'){
            $query="UPDATE rt_mst set post = 'Y' WHERE co_code='$value[0]'  AND doc_year ='$value[1]' AND doc_type ='$value[2]' AND doc_no='$value[3]' ";    
            $result = $conn->query($query);
        }elseif($value[2]=='DC'){
            $query="UPDATE dc_mst set post = 'Y' WHERE co_code='$value[0]'  AND doc_year ='$value[1]' AND doc_type ='$value[2]' AND doc_no='$value[3]' ";    
            $result = $conn->query($query);
        }else{
            $query="UPDATE order_mst set post = 'Y' WHERE co_code='$value[0]'  AND doc_year ='$value[1]' AND doc_type ='$value[2]' AND doc_no='$value[3]' ";    
            $result = $conn->query($query);
        }
    }
  // die();
