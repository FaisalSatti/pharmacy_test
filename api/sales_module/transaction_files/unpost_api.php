...
if(isset($_POST['action']) && $_POST['action'] == 'insert'){
  ...
  if($company_code !='0'){
    if($type !='0'){
        $merge=" AND co_code='$company_code' AND doc_type='$type' AND doc_date BETWEEN '$from_date' AND '$to_date' ";
    }else{
        $merge=" AND co_code='$company_code' AND doc_date BETWEEN '$from_date' AND '$to_date' ";
    }
}
  $query = "SELECT doc_date,doc_year,co_code,doc_no,doc_type,remarks FROM order_mst WHERE post = 'Y' $merge
          union ALL 
          SELECT doc_date,doc_year,co_code,doc_no,doc_type,remarks FROM dc_mst WHERE post = 'Y' $merge
          union ALL 
          SELECT doc_date,doc_year,co_code,doc_no,doc_type,remarks FROM rt_mst WHERE post = 'Y' $merge";
    // PRINT_R($query); die();
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data=[];
  ...
  foreach($extract as $detail){
      $value=explode("-",$detail);
      if($value[2]=='RT'){
        $query="UPDATE rt_mst set post = 'N' WHERE co_code='$value[0]'  AND doc_year ='$value[1]' AND doc_type ='$value[2]' AND doc_no='$value[3]' ";    
        $result = $conn->query($query);
    }elseif($value[2]=='DC'){
        $query="UPDATE dc_mst set post = 'N' WHERE co_code='$value[0]'  AND doc_year ='$value[1]' AND doc_type ='$value[2]' AND doc_no='$value[3]' ";    
        $result = $conn->query($query);
    }else{
        $query="UPDATE order_mst set post = 'N' WHERE co_code='$value[0]'  AND doc_year ='$value[1]' AND doc_type ='$value[2]' AND doc_no='$value[3]' ";    
        $result = $conn->query($query);
    }
  }
  // die();
