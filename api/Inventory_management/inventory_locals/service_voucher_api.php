...
if (isset($_POST['action']) && $_POST['action'] == 'company_code') {
    ...
    $query = "SELECT dept_code,dept_name FROM dept_store where substr(dept_code,1,3)=substr('$account_code',1,3) order by dept_name";
    // PRINT_R($query); DIE();  
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data = [];
    ...
    $select_q = "SELECT (case when MAX(voucher_no) is null then 1 else MAX(voucher_no)+1 end) voucher_no 
        FROM acttran_mst  WHERE co_code = '$company_code' AND doc_year = '$year' AND voucher_type = 'RV'";
    $select_r = $conn->query($select_q);
    $show = mysqli_fetch_assoc($select_r);
    $voucher_no = $show['voucher_no'];
    $master_q = "insert into acttran_mst(voucher_no,voucher_type,voucher_date,ref_no,co_code,doc_year,naration,payee, POST)value ('$voucher_no','RV','$voucher_date','$ref_no','$company_code','$year','$narration','$monthwise_ser_no','N')";
    ...
    if ($master_r) {
        ...
        for ($i = 0; $i < count($_POST['acc_desc']); $i++) {
            ...
            $detail_q = "insert into acttran_dtl(co_code,doc_year,voucher_type,voucher_no,voucher_date,account_code,debit,remarks,dept_code,vehicle_code,credit,entry_no)value ('$company_code','$year','RV','$voucher_no','$voucher_date','$acc_desc','$debit','$memo','$depart_desc','$vehicle_code','$credit','$entry_no')";
            ...
            if ($detail_r) {
                $return_data = array(
                    ...
                    "message" => "Service Voucher has been Inserted having Voucher No:" . $voucher_no
                );
            } else {
                $return_data = array(
                    "status" => 0,
                    "message" => "Voucher has not been Inserted"
                );
            }
            $entry_no++;
        }
    ...
    // print_r("jfsksk");
    $query = "SELECT * FROM acttran_mst WHERE voucher_type='RV'";
    // PRINT_R($query);
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data = [];
    ...
    $monthwise_ser_no = $_POST['monthwise_ser_no'];
    $voucher_no =  $_POST['voucher_no'];
    $upd_mst_q = "UPDATE acttran_mst SET voucher_date='$voucher_date', doc_year='$year' ,co_code='$co_code',ref_no='$ref_no',payee='$monthwise_ser_no',naration='$narration' WHERE voucher_no='$voucher_no'";
    // print_r($upd_mst_q);die();
    $upd_mst_r = $conn->query($upd_mst_q);
    if ($upd_mst_r) {
        $del_dtl_q = "DELETE FROM acttran_dtl WHERE co_code='$co_code' AND voucher_type='RV' AND doc_year='$year' AND voucher_no='$voucher_no'";
        ...
        if ($del_dtl_r) {
            ...
            for ($i = 0; $i < count($_POST['acc_desc']); $i++) {
                ...
                $detail_q = "insert into acttran_dtl(co_code,doc_year,voucher_type,voucher_no,voucher_date,account_code,debit,remarks,dept_code,vehicle_code,credit,entry_no)value ('$co_code','$year','RV','$voucher_no','$voucher_date','$acc_desc','$debit','$memo','$depart_desc','$vehicle_code','$credit','$entry_no')";
                // print_r($detail_q);die();
                $detail_r = $conn->query($detail_q);
                if ($detail_r) {
                    $return_data = array(
                        "status" => 1,
                        // "voucher_no" => $voucher_no,
                        "message" => "Service Voucher has been Updated having Voucher No:" . $voucher_no
                    );
                } else {
                    $return_data = array(
                        "status" => 0,
