...
if(isset($_POST['action']) && $_POST['action'] == 'insert'){
  ...
  if($company_code !='0'){
    if($type !='0'){
        ...
        $merge=" AND co_code='$company_code' AND doc_date BETWEEN '$from_date' AND '$to_date' ";
            ...
            $query = "SELECT DOC_DATE,DOC_YEAR,CO_CODE,DOC_NO,DOC_TYPE,REMARKS FROM po_mst_um WHERE post = 'Y' $merge
            union ALL 
            SELECT DOC_DATE,DOC_YEAR,CO_CODE,DOC_NO,DOC_TYPE,REMARKS FROM bill_mst_um WHERE post = 'Y' $merge";
    // PRINT_R($query); die();
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data=[];
  ...
  foreach($extract as $detail){
      $value=explode("-",$detail);
      if($value[2]=='LB'){
        $query="UPDATE bill_mst_um set post = 'N' WHERE CO_CODE='$value[0]'  AND DOC_YEAR ='$value[1]' AND DOC_TYPE ='$value[2]' AND DOC_NO='$value[3]' ";    
        $result = $conn->query($query);
    }else{
        $query="UPDATE po_mst_um set post = 'N' WHERE CO_CODE='$value[0]'  AND DOC_YEAR ='$value[1]' AND DOC_TYPE ='$value[2]' AND DOC_NO='$value[3]' ";    
        $result = $conn->query($query);
    }
  }
  // die();
