...
elseif(isset($_POST['action']) && $_POST['action'] == 'company_code'){
  ...
  elseif(isset($_POST['action']) && $_POST['action'] == 'item_detail_tr'){ 
    ...
    $item_code=$_POST['item_code'];
    $query = "SELECT b.gen_name, c.unit_name,e.div_name,  z.BAL_STOCK, L.loc_name,d.qty
    ...
    and d.item_code=a.item_div and a.co_code=d.co_code
...
elseif(isset($_POST['action']) && $_POST['action'] == 'item_code_ins'){
  $company_code=$_POST['company_code'];
  $query = "SELECT
            -- ITEM_NAME,div_name,gen_name,ITEM,CO_CODE,DIV_CODE
            a.*
            FROM ITEM_DETAIL_UM a 
...
elseif(isset($_POST['action']) && $_POST['action'] == 'item_code'){
  ...
  $query = "SELECT
            -- ITEM_NAME,div_name,gen_name,ITEM,CO_CODE,DIV_CODE
            a.*
            FROM ITEM_DETAIL_UM a LEFT join strtran_dtl b on a.item=b.item_code and a.co_code=b.co_code
            WHERE a.CO_CODE = '$company_code' and b.doc_type='ISLN'
            ORDER BY a.ITEM_NAME";
  $result = $conn->query($query);
  $count = mysqli_num_rows($result);
  $return_data=[];
...
elseif(isset($_POST['action']) && $_POST['action'] == 'loc_code'){
  ...
  $query = "SELECT A.*, B.ITEM_NAME, C.loc_name
            ...
            JOIN ITEM_DETAIL_UM B on A.CO_CODE = B.CO_CODE and A.item_code=B.item_code
...
elseif(isset($_POST['action']) && $_POST['action'] == 'account_detail'){
  ...
  $select_q="SELECT (case when MAX(DOC_NO) is null then 1 else MAX(DOC_NO)+1 end) DOC_NO 
            FROM strtran_mst  WHERE co_code = '$company_code' AND doc_year = '$year' and doc_type='ISLN' ";
  ...
  value ('$doc_no','ISLN','$voucher_date','$year','$company_code','$company_ref','$party','$party_ref','$division','$narration','Admin','$date','N' ,'L', '$company_code-99-L-$doc_no')";
  ...
  if($master_r){
      ...
      for($i=0;$i< count($_POST['acc_desc']); $i++){
          ...
          value ('$doc_no','ISLN','$voucher_date','$year','$company_code','$division','$narration','L', '$company_code-99-L-$doc_no','$acc_desc','$loc','$batch_i','$expirydt_i','$quantity','$rate','$amount' )"; 
          ...
          if($detail_r){
            ...
            $count = mysqli_num_rows($select_batch_r);
              ...
              $update_batch_q="UPDATE item_batch_no set ISS_STOCK='$batch_final_iss_qty', BAL_STOCK='$batch_final_qty' WHERE CO_CODE='$company_code' AND ITEM_CODE='$acc_desc'
              AND LOC_CODE='$loc' AND BATCH_NO='$batch_i' AND EXPIRY_DATE='$expirydt_i'";
    ...
    $query = "SELECT * FROM strtran_mst where DOC_TYPE = 'ISLN' ORDER BY doc_date desc,doc_no desc";
    $result = $conn->query($query);
    $count = mysqli_num_rows($result);
    $return_data=[];
      while($show = mysqli_fetch_assoc($result)){
...
elseif(isset($_POST['action']) && $_POST['action'] == 'update'){
  ...
  $upd_mst_q="UPDATE strtran_mst SET doc_date='$voucher_date', doc_year='$year' , remarks='$narration'
               WHERE doc_no='$doc_no' and doc_type = 'ISLN' and doc_year='$year' and co_code='$company_code' and do_key='$company_code-99-L-$doc_no'";
  $upd_mst_r = $conn->query($upd_mst_q);
  if($upd_mst_r){
    $min_qty_detail="SELECT qty from strtran_dtl where co_code = '$company_code' and doc_type = 'ISLN' and doc_year='$year' 
    ...
    WHERE doc_no='$doc_no' and doc_type = 'ISLN' and doc_year='$year' and co_code='$company_code' and do_key_ref='$company_code-99-L-$doc_no'";
      ...
      for($i=0;$i< count($_POST['acc_desc']); $i++){
        ...
        value ('$doc_no','ISLN','$voucher_date','$year','$company_code','$division','$narration','L', '$company_code-99-L-$doc_no','$acc_desc','$loc','$batch_i','$expirydt_i','$quantity','$rate','$amount' )"; 
            ...
            if($detail_r){
              ...
              $return_data = array(
                  "status" => 1,
                  "message" => "Issues [Loan To Party] - has been Updated having Doc No:".$doc_no 
              );
            }else{
              $return_data = array(
              "status" => 0,
              "message" => "Issues [Loan To Party] - has not been Updated"
              );
          }
      }
    }
