          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item"><button class="btn btn-sm" id="cs_breadcrumb">City Setup</button></li>
              </ol>
            </div>   
          </div>
        </div><!-- /.container-fluid -->
...
<div class="modal fade" id="InsertFormModel" tabindex="-1" aria-labelledby="exampleModalLabel" aria-modal="true"
            ...
            <div class="modal-body">
                <form method = "post" id = "city_form">
                    ...
                    <div class="col-md-6 form-group">
                        <label for="inputcityCode">Code :</label><span style="color:red;">*</span>
                        <div class="input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text"><i class="fas fa-arrow-up"></i></span>
                            </div>
                            <input pattern="[a-zA-Z0-9 ,._-]{1,}" min="0" maxlength="30" type="number" style="box-shadow: rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset;border:1px solid black;border-radius:4px" name="cityCode" id="cityCode" class="form-control form-control-sm" placeholder="city Code" > 
                        </div>
                    </div>
                    <div class="col-sm-6 form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input  maxlength="30" type="text" name="description" style="box-shadow: rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset;border:1px solid black;border-radius:4px" id="description" class="form-control form-control-sm" placeholder="Description" >
                        </div>
                    </div>
                    </div>
                    <div class="row">
  ...
  <div class="modal fade" id="EditFormModel" tabindex="-1" aria-labelledby="exampleModalLabel" aria-modal="true"
      ...
      <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLabel">Edit City Details</h5>
                  <button type="button" class="close" aria-label="Close" data-dismiss="modal">
                      <span aria-hidden="true">×</span>
                  </button>
              </div>
                  ...
                  <div class="row">
                    <div class="col-md-6 form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input pattern="[a-zA-Z0-9 ,._-]{1,}" min="0" maxlength="30" type="number" style="background-color:#ccd4e1;font-weight:bold;" name="cityCode_e" tabindex="-1" id="cityCode_e" class="form-control form-control-sm" placeholder="city Code" readonly> 
                        </div>
                    </div>
                    <div class="col-sm-6 form-group">
                        <label for="inputdescription">Description:</label><span style="color:red;">*</span>
...
$(function () {
  ...
  $('#city_form').validate({
    ...
    messages: {
      cityCode: {
        required: "Please enter a city code",
      },
      description: {
...
$(document).ready(function(){
    ...
    $.ajax({
        url: 'api/setup/city_setup_api.php',
        type:'POST',
        data: {action:'view'},
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            btn_edit = '<button class="btn btn-sm btn-select 702a1b_2" data-id="'+data[i].city_code+'" ><i class="far fa-edit text-info fa-fw"></i></button>';
            // btn_dlt = '<button class="btn btn-sm btn-delete" data-id="'+data[i].ID+'"><i class="far fa-trash-alt text-danger fa-fw"></i></button>';
            btn = btn_edit; 
            table.row.add([sno,data[i].city_code,data[i].city_name,btn]);
        }
        table.draw();
        },
        error: function(e){
    ...
    $("#cityCode").change(function(){
        ...
        $.ajax({
          url: 'api/setup/city_setup_api.php',
          type: 'POST',
          data: {action:'check_city_code',inputcityCode:cityCode},
          success: function (response) {
            console.log(response);
            if(response.status == "0"){
              $("#cityCode").css("background-color", "pink");
              $("#msg1").text(response.message);
              $("#cityCode").attr('placeholder',cityCode);
              $("#cityCode").val("");
            }else{
              $("#cityCode").css("background-color", "");
              $("#msg1").text("");
            }
          },
          error: function(e) 
...
$('#add_button').click(function(){
  ...
  $('#cityCode').focus();
    ...
    $("#city_form").on('click','#insert',function(){    
      if ($("#city_form").valid()) {
            $("#ajax-loader").show();
          var city_code = $('#cityCode').val();
          var city_name = $('#description').val();
          
          $.ajax({
              url: 'api/setup/city_setup_api.php',
              type:'POST',
              data :  {action:'insert',city_code:city_code,city_name:city_name},
              success: function(response)
              {
                $("#ajax-loader").hide();
...
//edit
  $("#example1").on('click','.btn-select',function(){
    ...
    $.ajax({
        url : 'api/setup/city_setup_api.php',
        type : "post",
        data : {action:'edit',city_code:city_code},
        success: function(response)
        {console.log(response);
            $('#cityCode_e').val(response.city_code);
            $('#description_e').val(response.city_name);
  ...
  $('#description_e').focus();
    ...
    //update
      $("#EditForm").on('click','#update_data',function(){
        if ($("#EditForm").valid())
        { 
          var city_code = $('#cityCode_e').val();
          var description = $('#description_e').val();
          
          $.ajax({
                  url: 'api/setup/city_setup_api.php',
                  type:'POST',
                  data :  {action:'update',city_code:city_code,description:description},
                  success: function(response)
                  {
                    var message = response.message
                    var status = response.status
