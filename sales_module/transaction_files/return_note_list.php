          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item"><button class="btn btn-sm" id="return_note_breadcrumb">Return Note List</button></li>
              </ol>
            </div>   
          </div>
        </div><!-- /.container-fluid -->
...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        //   console.log(narration);
            btn_edit = '<button class="btn btn-sm btn-select 702a1b_2" do_key="'+data[i].do_key+'" div_code="'+data[i].div_code+'" party_code="'+data[i].party_code+'" salesman="'+data[i].sman_code+'"  po_catg="'+data[i].po_catg+'"  doc_date="'+data[i].doc_date+'" narration="'+data[i].remarks+'" co_code="'+data[i].co_code+'" doc_year="'+data[i].doc_year+'" doc_type="'+data[i].doc_type+'" doc_no="'+data[i].doc_no+'" ><i class="far fa-edit text-info fa-fw"></i></button>';
...
$("#example1").on('click','.btn-select',function(){
    ...
    $.get('sales_module/transaction_files/return_note_edit.php?co_code='+co_code+'&doc_no='+doc_no+'&po_catg='+po_catg+'&doc_year='+doc_year+'&party_code='+party_code+'&div_code='+div_code+'&salesman_code='+salesman_code+'&do_key='+do_key, function(data,status){
        $("#content").html(data);
    });
});
...
$("#return_note_breadcrumb").on("click", function () {
