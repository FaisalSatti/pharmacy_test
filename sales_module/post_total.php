                  ...
                  <div class="row">
                    ...
                    <div class="row p-1">
                      <div class="col-md-12 text-right">
                          <button type="submit" class="btn btn-primary bulk_inv_btn" ><i class="fas fa-print fa-fw"></i>Post Vouchers</button>
                      </div>
                    </div>
                  </div> 
                  <div class="row">
...
<script>
  ...
  $("document").ready(function(){
    ...
    $.ajax({
      url: 'api/sales_module/transaction_files/post_api.php',
      type:'POST',
      data: {action:'view2',company_code:company_code,type:type,from_date:from_date,to_date:to_date},
                        ...
                        var status = response.status
                          $.ajax({
                            ...
                            if(status == 0){
                                ...
                                $.get('sales_module/post_total.php',function(data,status){
                        ...
                        $.ajax({
                          ...
                          success: function(data) {
                            //   console.log(data.length);
                              ...
                              var narration=data[i].remarks=='' ?'-':data[i].remarks;
                                    // console.log(data[i].NARATION);
                                  btn_edit = '<button class="btn btn-sm btn-select 702a1b_2" co_code="'+data[i].co_code+'" voucher_year="'+data[i].doc_year+'" voucher_type="'+data[i].doc_type+'" voucher_no="'+data[i].doc_no+'" ><i class="far fa-edit text-info fa-fw"></i></button>';
                              // btn_dlt = '<button class="btn btn-sm btn-delete" data-id="'+data[i].ID+'"><i class="far fa-trash-alt text-danger fa-fw"></i></button>';
                              btn = btn_edit; 
                              table.row.add([sno,data[i].doc_date,data[i].DOC_YEAR,data[i].CO_CODE,data[i].VOUCHER_NO,data[i].doc_type,narration,checkboxes+' <b style="color:#1f1f87">Posted</b>']);
                              }
                              table.draw();
                          },
                          error: function(e){
