    ...
    //    print_r($query);die();
          ...
          while($show=$result->fetch_assoc()){
            ...
            while($show1=$result1->fetch_assoc()){
              ...
              $subControlCode = $show1['ACCOUNT_CODE'];
              foreach($accountCode2 as $acc_no) {
              
                if($subControlCode == $acc_no){
                  $test = 1;
                  break;
                }else{
                    $test = 0;
            ...
            //   print_r($test);die();
              if($test == 1){
                ...
                while($show2=$result2->fetch_assoc()){
                  ...
                  $query4 = "SELECT SUM(debit) AS Totaldebit, SUM(credit) As totalcredit  FROM MASTER_VIEW_LEDGER WHERE co_code = '$company_code' AND account_code = '$subsdiaryCode' AND voucher_date between '$from_date' and '$to_date' GROUP BY account_code";
