...
$query = "SELECT DOC_TYPE,DOC_NO,DOC_DATE,DO_KEY,PUR_MODE,ITEM_CAT,
        ...
        $merge_for_party $merge_for_item
...
if ($count > 0) {
    ...
    foreach ($return_data as $value) {
        $party_code_new=$value['PARTY_CODE'];
        if($party_code_pre != $party_code_new){
            if($i !=1){
                $data_tr .='</table>
    ...
    $mpdf->WriteHTML('
        <div class="row" style="line-height:16px;font-family:arial;">
            ...
            <table style="border-bottom: 4px solid black; padding:5px;width:100%;">
                <tr>
                    <td style = "font-size:15px ;line-height:20px;width:28%; text-align:center;">Itemwise Purchase/Return Report : </td>
